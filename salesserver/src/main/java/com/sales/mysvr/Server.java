...
import com.sales.mysvr.base.tags.TagLookupOverride;
...
public class Server {
	...
	private byte[] fetchKey() {
		...
		String base64Key = "SzNp20JJ+yDxwXT1ZRedN6FHfL6XZeV35GwinHU0XY0hW/HP/k0qlTzvIyIvrMuD6LnIq0Y1VqMkIWgqjjhTIQ==";  //This gets replaced with a unique key each generated project which you need to keep or replace with your own!!!
	...
	private File modifyUserDirForManyEnvironmentsImpl(File filePath) {
		...
		if("salesserver-all".equals(name)) {
			return FileFactory.newFile(filePath, "salesserver/src/dist");
		} else if("salesserver-dev".equals(name)) {
			File parent = filePath.getParentFile();
			return FileFactory.newFile(parent, "salesserver/src/dist");
		} else if(!"salesserver".equals(name)) {
			if(FileFactory.endsWith(filePath, "salesserver/src/dist"))
				return filePath; //This occurs when a previous test ran already and set user.dir
			else if(name.equals("webpieces"))
				return FileFactory.newFile(filePath, "webserver/webpiecesServerBuilder/templateProject/salesserver/src/dist");
